...
# ==========================================
def main():
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 監視処理を開始します。")
    conn, is_initial_setup = init_db()
    c = conn.cursor()

    if is_initial_setup:
        print("【安全装置発動】DB初回作成のため、全件既読化処理を行います（LINE通知は送信しません）。")
    ...
    try:
        ...
        urls_to_check = list(
            set(
                [
                    a["href"] if a["href"].startswith("http") else f"https://www.kanritsuriba.com{a['href']}"
                    for a in links
                ]
            )
        )
    ...
    for url in urls_to_check:
        try:
            ...
            content_area = soup_detail.find("div", class_="entry-content") or soup_detail
            ...
            if not row:
                ...
                if entry_dt and entry_dt > now and info["is_cancelled"] == 0:
                    ...
                    if timedelta(0) < time_diff <= timedelta(days=1) and not n_1d:
                        ...
                        db_updates.append(
                            ("UPDATE tournaments SET notified_1d = 1 WHERE url = ?", (url,))
                        )
                    ...
                    # 1時間前通知
                        ...
                        db_updates.append(
                            ("UPDATE tournaments SET notified_1h = 1 WHERE url = ?", (url,))
                        )

                    # 10分前通知
                    elif timedelta(0) < time_diff <= timedelta(minutes=10) and not n_10m:
                        notify_queue.append(
                            f"🔥【10分前直前通知】\n第{db_round}戦 {db_loc}大会\nエントリー準備をしてください。\nURL: {url}"
                        )
                        db_updates.append(
                            ("UPDATE tournaments SET notified_10m = 1 WHERE url = ?", (url,))
                        )
    ...
    if not is_initial_setup and notify_queue:
        if len(notify_queue) > MAX_NOTIFY_LIMIT:
            print(f"⚠️ 通知件数が制限({MAX_NOTIFY_LIMIT}件)を超えたため個別送信をスキップします。")
            send_line_message(
